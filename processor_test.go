...
func TestProcessor(t *testing.T) {
	...
	doGoldenTests(t, filepath.Join(testDataDir, "golden/result"), []test{
		...
		{name: "gif frames", path: "fit-in/100x100/filters:gif(4)/everybody-betray-me.mkv"},
		{name: "gif frames fps", path: "fit-in/100x100/filters:gif(4,2)/everybody-betray-me.mkv"},
		{name: "gif clip", path: "fit-in/100x100/filters:seek(1s):gif(1s,4)/everybody-betray-me.mkv"},
		{name: "gif clip position", path: "fit-in/100x100/filters:seek(0.5):gif(500ms,10)/schizo_90.mp4"},
		{name: "gif alpha webp", path: "fit-in/100x100/filters:gif(3):format(webp)/alpha-webm.webm"},
		{name: "gif static png", path: "fit-in/100x100/filters:gif(3):format(png)/everybody-betray-me.mkv"},
		{name: "gif invalid args", path: "fit-in/100x100/filters:gif(abc)/everybody-betray-me.mkv"},
		{name: "gif audio only 406", path: "fit-in/100x100/filters:gif(3)/no_cover.mp3", expectCode: 406},
